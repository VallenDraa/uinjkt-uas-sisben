...
export const BABY_MONITORING_AUDIO_WS_URL =
  getEnv("WS_URL") + "/baby-monitoring/audio";

export const useBabyMonitoringAudioWebSocket = (hardwareId: string) => {
  const socket = useWebSocket<BabyMonitoringNotification | null>(
    `${BABY_MONITORING_AUDIO_WS_URL}/${hardwareId}/`,
    WEBSOCKET_CONFIG,
  );
